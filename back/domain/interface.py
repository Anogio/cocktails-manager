...
def get_cocktails_for_ingredients(
    available_ingredients: list[str], susbstitute: bool,
    ...
    if len(available_ingredients) == 0:
        ...
        possible_cocktails = [cocktail for cocktail in cocktails if cocktail_score(cocktail, ingredients) > 0]
        res = sorted(possible_cocktails, key=lambda c: cocktail_score(c, ingredients), reverse=True)
